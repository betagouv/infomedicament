...
describe("utils presentations", () => {
  it("getPresentationName - when PVC-Alumunium + PVC in the details only display PVC-Aluminium", async () => {
    const presentations: Presentation[] =
      await getFullPresentations("66150367");
    const fullPresentationName: string = getPresentationName(presentations[0]);
    const shortPresentationName: string = getPresentationName(
      presentations[0],
      true,
    );
    expect(fullPresentationName).toBe(
      "1 plaquette PVC-Aluminium de 30 gélules",
    );
  ...
  it("getPresentationName - quantity with comma", async () => {
    const presentations: Presentation[] =
      await getFullPresentations("66663761");
    const fullPresentationName: string = getPresentationName(presentations[0]);
    const shortPresentationName: string = getPresentationName(
      presentations[0],
      true,
  ...
  it("getPresentationName - plural", async () => {
    const presentations: Presentation[] =
      await getFullPresentations("64783769");
    const fullPresentationName: string = getPresentationName(presentations[0]);
    const shortPresentationName: string = getPresentationName(
      presentations[0],
      true,
    ...
    expect(fullPresentationName).toBe(
      "2 stylos préremplis de 0,4 ml avec tampon alcoolisé",
    );
  ...
  it("getPresentationName - only display recipients details if on the original name", async () => {
    const presentations: Presentation[] =
      await getFullPresentations("60184188");
    const fullPresentationName: string = getPresentationName(presentations[0]);
    const shortPresentationName: string = getPresentationName(
      presentations[0],
      true,
  ...
  it("getPresentationName - recipients details list must be unique", async () => {
    const presentations: Presentation[] =
      await getFullPresentations("60018444");
    const fullPresentationName: string = getPresentationName(presentations[0]);
    const shortPresentationName: string = getPresentationName(
      presentations[0],
      true,
    ...
    expect(fullPresentationName).toBe(
      "30 plaquettes aluminium OPA : polyamide orienté PVC-Aluminium de 1 comprimé",
    );
  ...
  it("getPresentationName - dispositif is displayed at the end of everything", async () => {
    const presentations: Presentation[] =
      await getFullPresentations("63886766");
    const fullPresentationName: string = getPresentationName(presentations[0]);
    const shortPresentationName: string = getPresentationName(
      presentations[0],
      true,
    ...
    expect(fullPresentationName).toBe(
      "4 flacons - 4 seringues préremplies avec aiguille avec adaptateur pour flacon avec tampon alcoolisé",
    );
  ...
  it("getPresentationName - multiple presentations", async () => {
    const presentations: Presentation[] =
      await getFullPresentations("60052222");

    const fullPresentationNameA: string = getPresentationName(presentations[0]);
    const shortPresentationNameA: string = getPresentationName(
      presentations[0],
      true,
    );
    expect(fullPresentationNameA).toBe(
      "1 flacon en verre brun de 13,2 ml avec dispositif pulvérisateur",
    );
    ...
    const shortPresentationNameB: string = getPresentationName(
      presentations[1],
      true,
    );
    expect(fullPresentationNameB).toBe(
      "2 flacons en verre brun de 13,2 ml avec dispositif pulvérisateur",
    );
  ...
  it("getPresentationName - multiple recipients", async () => {
    const presentations: Presentation[] =
      await getFullPresentations("60206332");
    const fullPresentationName: string = getPresentationName(presentations[0]);
    const shortPresentationName: string = getPresentationName(
      presentations[0],
      true,
    ...
    expect(fullPresentationName).toBe(
      "1 plaquette PVC-Aluminium PVDC de 12 comprimés - 1 plaquette PVC-Aluminium PVDC de 4 comprimés",
    );
    expect(shortPresentationName).toBe(
      "Plaquette de 12 comprimés - Plaquette de 4 comprimés",
    );
  });

  it("getFullPresentations - abrogée status", async () => {
    const presentationsA: Presentation[] =
      await getFullPresentations("69174918");
    expect(isAbrogee(presentationsA[0])).toBe(true);
    const presentationsB: Presentation[] =
      await getFullPresentations("62772966");
    expect(isAbrogee(presentationsB[0])).toBe(false);
  });

  it("getFullPresentations - arrêtée status", async () => {
    const presentationsA: Presentation[] =
      await getFullPresentations("65089833");
  ...
  it("getFullPresentations - not authorized status", async () => {
    const presentationsA: Presentation[] =
      await getFullPresentations("64460075");
  ...
  it("getFullPresentations - agréée status", async () => {
    const presentationsA: Presentation[] =
      await getFullPresentations("66296030");
  ...
  it("getFullPresentations - liste sus status", async () => {
    const presentationsA: Presentation[] =
      await getFullPresentations("60199966");
  ...
  it("getFullPresentations - IVG status", async () => {
    const presentationsA: Presentation[] =
      await getFullPresentations("69981979");
