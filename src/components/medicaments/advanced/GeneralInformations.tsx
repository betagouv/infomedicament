...
import {
  getPresentationName,
  getPresentationFullPriceText,
  getPresentationCommercialStatusLabel,
  isAbrogee,
  isAgree,
  isIVG,
  isListeRetrocession,
...
import { FicheInfos, ImportantInformation } from "@/types/FicheInfoTypes";
...
function SummaryLine({
  ...
  return (
    <SummaryLineContainer
      className={fr.cx("fr-mb-1w", "fr-pb-1w", "fr-mt-1w", "fr-text--sm")}
...
function GeneralInformations({
  ...
  return (
    ...
    specialite && (
      <div {...props}>
        {ficheInfos.listeInformationsImportantes &&
          displayInfosImportantes(ficheInfos) && (
            <ContentContainer
              id="informations-importantes"
              whiteContainer
              className={fr.cx("fr-mb-2w", "fr-p-2w")}
            >
              <h2 className={fr.cx("fr-h6")}>Informations importantes</h2>
              {ficheInfos.listeInformationsImportantes.map(
                (info: ImportantInformation, index) => {
                  return (
                      ...
                      {(info.eventDate || info.typeLabel) && (
                        <div className={fr.cx("fr-mt-1w")}>
                          {info.eventDate && (
                            ...
                            >
                              {info.eventDate.toLocaleDateString("fr-FR", {
                                month: "long",
                                year: "numeric",
                              })}
                            </i>
        ...
        >
          ...
          <SummaryLine categoryName="Grossesse">
            {isPregnancyMentionAlert || isPregnancyPlanAlert ? (
              ...
              <span>
                Pas de contre-indication grossesse stricte, vérifier en 4.3 et
                4.6 du RCP
              </span>
            )}
          </SummaryLine>
          ...
          <SummaryLine categoryName="Date d'autorisation de mise sur le marché">
            {specialite.SpecDateAMM ? (
              <span>
                Le&nbsp;{specialite.SpecDateAMM.toLocaleDateString("fr-FR")}
              </span>
            ) : (
          ...
          <SummaryLine
            categoryName="Conditions de prescription et de délivrance"
            hideBorder
          >
                      ...
                      <li key={line.code || index}>
                        {label === "liste I" || label === "liste II" ? (
                          <WithDefinition
                            definition={
                              definitions &&
                              getDefinition(definitions, "Liste I et II")
                            }
                            word={label}
                          />
                        ) : (
                ...
                return (
                  <div key={index} className={fr.cx("fr-mb-2w")}>
                    <div className={fr.cx("fr-mb-1w")}>
                      {element.referenceDosage.charAt(0).toUpperCase()}
                      {element.referenceDosage.substring(1)}
                    </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          {composant.composants &&
                            composant.composants.length > 0 && (
                              <div>
                                {composant.composants.map(
                                  (subComposant, indexSubComposant) => {
                                    return (
                                      <div
                                        key={indexSubComposant}
                                        className={fr.cx("fr-ml-3w")}
                                      >
                                        sous forme de : {subComposant.NomLib}{" "}
                                        {subComposant.dosage}
                                      </div>
                                    );
                                  },
        ...
        {stocks.length > 0 && (
          <ContentContainer
            id="informations-stock"
            whiteContainer
            className={fr.cx("fr-mb-2w", "fr-p-2w")}
          >
            <h2 className={fr.cx("fr-h6")}>
              Ruptures de stock ou risques de rupture de stock
            </h2>
            {stocks.map((stock, index) => (
              <StockBlock key={index} $hideBorder={index === stocks.length - 1}>
                ...
                <SummaryLine categoryName="Statut">
                  <StockTag statusId={stock.status_id} />
                </SummaryLine>
