...
scalingo -a "$APP" --region "$REGION" run "for x in indications substances specialites generiques medicaments; do node .next/standalone/scripts/updateResumeData.js \$x; done && node .next/standalone/scripts/seedSearchIndex.js"
